...
pub fn read_file(path: &Path, file_type: FileTypeCheck) -> IoResult<Vec<u8>> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // Open FIFOs without waiting for a writer so descriptor validation can reject them.
        open_options.custom_flags(libc::O_CLOEXEC | libc::O_NOFOLLOW | libc::O_NONBLOCK);
    }
...
fn open_new_output(path: &Path) -> io::Result<File> {
    ...
    {
        use std::os::unix::fs::OpenOptionsExt;
        // Keep incomplete output private until descriptor-based chmod after writing.
        options
            .mode(0o600)
...
mod review_tests {
    ...
    #[test]
    fn output_permissions_and_writes_stay_on_original_descriptor() {
        let root =
            std::env::temp_dir().join(format!("pdvzip-rust-descriptor-{}", std::process::id()));
        fs::create_dir(&root).unwrap();
        let output = root.join("temporary.png");
        let peer = root.join("original");
        let target = root.join("unrelated");
        let file = open_new_output(&output).unwrap();
        assert_eq!(file.metadata().unwrap().permissions().mode() & 0o777, 0o600);
        fs::hard_link(&output, &peer).unwrap();
        fs::write(&target, b"unchanged").unwrap();
        fs::set_permissions(&target, fs::Permissions::from_mode(0o600)).unwrap();
        fs::remove_file(&output).unwrap();
        symlink(&target, &output).unwrap();
        write_file_contents(file, &output, b"polyglot").unwrap();
        assert_eq!(fs::read(&peer).unwrap(), b"polyglot");
        assert_eq!(
            fs::metadata(&peer).unwrap().permissions().mode() & 0o777,
            0o755
        );
