...
const MAX_TOTAL_NORMALIZED_PATH_BYTES: usize = 1024 * 1024;
const MAX_PATH_NODES: usize = MAX_TOTAL_NORMALIZED_PATH_BYTES + 1;
...
const EXTRA_ASI_UNIX: u16 = 0x756e;
const EXTRA_LIBARCHIVE_XL: u16 = 0x6c78;
...
impl PortablePathTrie {
    fn with_entry_capacity(entry_count: usize) -> ArchiveResult<Self> {
        let initial_capacity = entry_count
            .min(MAX_TOTAL_NORMALIZED_PATH_BYTES)
            .checked_add(1)
    ...
    fn find_or_add_child(&mut self, parent: usize, label: u8) -> ArchiveResult<usize> {
        ...
        if self.nodes.len() == self.nodes.capacity() {
            if index >= MAX_PATH_NODES {
                return Err(
                    "Archive Security Error: Archive path trie exceeds its safety limit."
                        .to_string(),
                );
            }
            // Cap geometric growth explicitly, including the final allocation.
            let growth = self
                .nodes
                .capacity()
                .min(MAX_PATH_NODES - self.nodes.capacity());
            self.nodes.try_reserve_exact(growth).map_err(|_| {
...
fn validate_zip_extra_fields(
    ...
    while cursor < extra.len() {
        ...
        // Extractors can obtain replacement file types from these fields.
        if field_id == EXTRA_ASI_UNIX || field_id == EXTRA_LIBARCHIVE_XL {
            return Err(format!(
                "Archive Security Error: File-type override extra field 0x{field_id:04x} is not supported in {location} on entry {entry_number}."
            ));
        }
        cursor += field_size;
    }
...
fn validate_entry_attributes(entry: &CentralEntryMetadata<'_>) -> ArchiveResult<()> {
    ...
    // Extractors honor UNIX type bits even for foreign creator hosts.
...
fn classify_zip_filename(filename: &[u8]) -> ArchiveResult<FileType> {
    ...
    // Parent-directory dots do not supply the final component's extension.
    let basename = filename
        .rsplit(|byte| *byte == b'/')
        .next()
        .unwrap_or(filename);
    let Some(dot_position) = basename.iter().rposition(|byte| *byte == b'.') else {
        return Ok(FileType::LinuxExecutable);
    };
    let extension = &basename[dot_position + 1..];
...
/// Validate ZIP entries with a 1 MiB aggregate path-byte limit (excluding trailing directory slashes).
...
mod review_tests {
    ...
    #[test]
    fn path_trie_growth_stays_within_node_budget() {
        let mut trie = PortablePathTrie::with_entry_capacity(17).unwrap();
        for i in 0..16 {
            let name = format!("{i:02}{}", "a".repeat(65533));
            trie.insert(name.as_bytes(), i + 1).unwrap();
            assert!(trie.nodes.capacity() <= MAX_PATH_NODES);
        }
        trie.insert(b"zzzzzzzzzzzzzzzz", 17).unwrap();
        assert_eq!(
            trie.total_normalized_path_bytes,
            MAX_TOTAL_NORMALIZED_PATH_BYTES
        );
        assert!(trie.nodes.capacity() <= MAX_PATH_NODES);
        assert!(trie.insert(b"next", 18).unwrap_err().contains("1 MiB"));
    }
}
