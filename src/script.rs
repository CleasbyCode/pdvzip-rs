...
// ============================================================================
// v4.8 Linux extraction macros (verbatim from script_text_builder.cpp)
// ============================================================================

const LINUX_EXTRACT_ITEM: &str = r#"ITEM={{LINUX_FILENAME_ARG}};SELF=${0##*/};DIR=${SELF%.*};case $DIR in ''|.|..|"$SELF")DIR=${SELF}_files;;esac;PATH_DIR=./$DIR;clear;mkdir -- "$PATH_DIR"||exit;unzip -qo -d "$PATH_DIR" -- "${0/#-/./-}"||exit;[ -e "$PATH_DIR/$SELF" ]||[ -L "$PATH_DIR/$SELF" ]||mv -nT -- "$0" "$PATH_DIR/$SELF"||exit;cd "$PATH_DIR"||exit;"#;
const LINUX_EXTRACT_ITEM_HASH: &str = r#"ITEM={{LINUX_FILENAME_ARG}};SELF=${0##*/};DIR=${SELF%.*};case $DIR in ''|.|..|"$SELF")DIR=${SELF}_files;;esac;PATH_DIR=./$DIR;clear;mkdir -- "$PATH_DIR"||exit;unzip -qo -d "$PATH_DIR" -- "${0/#-/./-}"||exit;[ -e "$PATH_DIR/$SELF" ]||[ -L "$PATH_DIR/$SELF" ]||mv -nT -- "$0" "$PATH_DIR/$SELF"||exit;cd "$PATH_DIR"||exit;hash -r;"#;
const LINUX_EXTRACT_ITEM_HASH_NULL: &str = r#"ITEM={{LINUX_FILENAME_ARG}};SELF=${0##*/};DIR=${SELF%.*};case $DIR in ''|.|..|"$SELF")DIR=${SELF}_files;;esac;PATH_DIR=./$DIR;NUL="/dev/null";clear;mkdir -- "$PATH_DIR"||exit;unzip -qo -d "$PATH_DIR" -- "${0/#-/./-}"||exit;[ -e "$PATH_DIR/$SELF" ]||[ -L "$PATH_DIR/$SELF" ]||mv -nT -- "$0" "$PATH_DIR/$SELF"||exit;cd "$PATH_DIR"||exit;hash -r;"#;
const LINUX_EXTRACT_NO_ITEM: &str = r#"SELF=${0##*/};DIR=${SELF%.*};case $DIR in ''|.|..|"$SELF")DIR=${SELF}_files;;esac;PATH_DIR=./$DIR;clear;mkdir -- "$PATH_DIR"||exit;unzip -qo -d "$PATH_DIR" -- "${0/#-/./-}"||exit;[ -e "$PATH_DIR/$SELF" ]||[ -L "$PATH_DIR/$SELF" ]||mv -nT -- "$0" "$PATH_DIR/$SELF"||exit;cd "$PATH_DIR"||exit;"#;

// ============================================================================
// v4.8 Windows extraction macros (verbatim from script_text_builder.cpp)
// ============================================================================
...
fn get_script_template(file_type: FileType) -> ScriptTemplate {
    match file_type {
        ...
        FileType::Powershell => ScriptTemplate {
            linux_part: format!(
                "{LINUX_EXTRACT_ITEM_HASH}{}",
                r#"if command -v pwsh >/dev/null 2>&1;then clear;pwsh -File "$ITEM" {{LINUX_ARGS}};STATUS=$?;exit "$STATUS";else clear;printf '%s\n' 'pdvzip: required runtime pwsh was not found.' >&2;exit 127;fi;"#
            ),
            windows_part: format!(
...
fn split_posix_arguments(input: &[u8], field_name: &str) -> ScriptResult<Vec<Vec<u8>>> {
    ...
    for byte in input {
        ...
        if escaped {
            // Only these four bytes consume a backslash inside double quotes.
            if state == QuoteState::Double && !matches!(*byte, b'$' | b'`' | b'"' | b'\\') {
                current.push(b'\\');
            }
            current.push(*byte);
            escaped = false;
...
mod tests {
    ...
    fn templates_use_same_stem_and_report_missing_runtimes() {
        ...
        let powershell =
            build_extraction_script(FileType::Powershell, "app.ps1", &args).expect("powershell");
        let powershell_text = String::from_utf8_lossy(&powershell);
        assert!(powershell_text.contains(r#"pwsh -File "$ITEM""#));
        assert!(!powershell_text.contains(r#"pwsh "$ITEM""#));
    }
...
mod review_tests {
    ...
    #[test]
    fn double_quoted_backslashes_follow_posix_rules() {
        for (input, expected) in [
            ...
            (br#""\$\`\"\\""#, br#"$`"\"#),
            (br"'one\two'", br"one\two"),
            (br"one\ two", b"one two"),
            (br#""""#, b""),
        ] {
            assert_eq!(
                split_posix_arguments(input, "test").unwrap(),
                vec![expected.to_vec()]
            );
        }
    }
    ...
    #[test]
    fn comment_byte_in_chunk_length_is_padded() {
        for length in 0..256 {
            let args = UserArguments {
                linux_args: vec![b'a'; length],
                ..Default::default()
            };
            let text = build_script_text(FileType::LinuxExecutable, b"runner", &args).unwrap();
            // Profile name, compression marker, and shell prefix/suffix occupy 16 bytes.
            let unpadded_length = text.len() + 16;
            if unpadded_length & 255 != usize::from(b'#') {
                continue;
            }
            let chunk =
                build_extraction_script(FileType::LinuxExecutable, b"runner", &args).unwrap();
            let data_length = u32::from_be_bytes(chunk[..4].try_into().unwrap()) as usize;
            assert_eq!(data_length, unpadded_length + 8);
            assert!(!is_linux_problem_metacharacter(chunk[3]));
            assert_eq!(data_length + 12, chunk.len());
            assert_eq!(
                crc32fast::hash(&chunk[4..chunk.len() - 4]),
                u32::from_be_bytes(chunk[chunk.len() - 4..].try_into().unwrap())
            );
            return;
        }
        panic!("no comment-byte length fixture found");
    }
}
