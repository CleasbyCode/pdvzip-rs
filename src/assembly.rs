...
fn find_zip_eocd(
    image_vec: &[u8],
    zip_base_offset: usize,
    zip_end_offset: usize,
) -> AssemblyResult<ZipEocdInfo> {
    const EOCD_MIN_SIZE: usize = 22;
    const EOCD_DISK_NUMBER_OFFSET: usize = 4;
    const EOCD_CENTRAL_DISK_OFFSET: usize = 6;
    const EOCD_RECORDS_ON_DISK_OFFSET: usize = 8;
    const EOCD_TOTAL_RECORDS_OFFSET: usize = 10;
    const EOCD_CENTRAL_SIZE_OFFSET: usize = 12;
    const EOCD_CENTRAL_OFFSET: usize = 16;

    if zip_end_offset > image_vec.len()
        || zip_end_offset < zip_base_offset
        || zip_end_offset - zip_base_offset < EOCD_MIN_SIZE
    {
        return Err("ZIP Error: Archive is too small.".to_string());
    }

    let locator = binary_utils::find_zip_eocd_locator(image_vec, zip_base_offset, zip_end_offset)
        .ok_or_else(|| {
        "ZIP Error: End of Central Directory signature not found.".to_string()
    })?;
    let position = locator.index;
    let info = ZipEocdInfo {
        index: position,
        total_records: read_le16(image_vec, position + EOCD_TOTAL_RECORDS_OFFSET)?,
        central_size: read_le32(image_vec, position + EOCD_CENTRAL_SIZE_OFFSET)?,
        central_offset: read_le32(image_vec, position + EOCD_CENTRAL_OFFSET)?,
        comment_length: locator.comment_length,
    };

    let disk_number = read_le16(image_vec, position + EOCD_DISK_NUMBER_OFFSET)?;
    let central_disk = read_le16(image_vec, position + EOCD_CENTRAL_DISK_OFFSET)?;
    let records_on_disk = read_le16(image_vec, position + EOCD_RECORDS_ON_DISK_OFFSET)?;
    if disk_number != 0
        || central_disk != 0
        || records_on_disk != info.total_records
        || info.total_records == u16::MAX
        || info.central_size == u32::MAX
        || info.central_offset == u32::MAX
    {
        return Err("ZIP Error: ZIP64 or multi-disk archives are not supported.".to_string());
    }
    ...
    if central_start > zip_end_offset || central_end > zip_end_offset || central_end != position {
...
fn fix_zip_offsets(
    ...
    const CENTRAL_DISK_START_OFFSET: usize = 34;
    ...
    if image_vec.len() < PNG_TRAILING_BYTES {
        return Err("ZIP Error: Embedded image is truncated.".to_string());
    }
    let zip_end_offset = image_vec.len() - PNG_TRAILING_BYTES;
    let eocd = find_zip_eocd(image_vec, zip_base_offset, zip_end_offset)?;
    let central_start = binary_utils::checked_add(
        zip_base_offset,
    ...
    for record_index in 0..usize::from(eocd.total_records) {
        ...
        let entry_disk_start = read_le16(image_vec, cursor + CENTRAL_DISK_START_OFFSET)?;
        let record_size = binary_utils::zip_central_directory_record_size(
            name_length,
            extra_length,
            comment_length,
        )?;
        ...
        if entry_disk_start != 0 {
            return Err(format!(
                "ZIP Error: Multi-disk local header reference on record {} is not supported.",
                record_index + 1
            ));
        }
...
pub fn embed_chunks(
    ...
    if archive_vec.len() < CHUNK_FIELDS_COMBINED_LENGTH {
    ...
    let additional_size = script_vec
        .len()
        .checked_add(archive_vec.len())
        .ok_or_else(|| "Embed Error: Output image size overflow.".to_string())?;
    image_vec
        .try_reserve_exact(additional_size)
        .map_err(|_| "Embed Error: Unable to allocate output image buffer.".to_string())?;
...
mod tests {
    ...
    fn rejects_archive_without_eocd() {
        let mut image = base_image();
        let script = include_bytes!("../tests/fixtures/assembly_zip_video.script.bin").to_vec();
        // The archive buffer includes the 12-byte PNG chunk wrapper. Keep the
        // payload large enough for a minimum ZIP EOCD so this specifically
        // exercises the missing-signature path instead of wrapper truncation.
        let archive = vec![0u8; 34];
    ...
    #[test]
    fn rejects_incomplete_archive_chunk_wrapper() {
        let mut image = base_image();
        let script = include_bytes!("../tests/fixtures/assembly_zip_video.script.bin").to_vec();
        let archive = vec![0u8; 11];
        let err = embed_chunks(&mut image, script, archive, base_image_size())
            .expect_err("expected wrapper truncation");
        assert!(err.contains("Archive data is truncated"));
    }

    #[test]
    fn rejects_eocd_gap_before_png_trailer() {
        let mut image = base_image();
        let script = include_bytes!("../tests/fixtures/assembly_zip_video.script.bin").to_vec();
        let mut archive =
            include_bytes!("../tests/fixtures/assembly_zip_video.archive.bin").to_vec();
        archive.insert(archive.len() - 4, 0x41);

        let err = embed_chunks(&mut image, script, archive, base_image_size())
            .expect_err("EOCD comment must end exactly at archive boundary");
        assert!(err.contains("End of Central Directory signature not found"));
    }

    #[test]
    fn rejects_multidisk_metadata_during_rebase() {
        let mut image = base_image();
        let script = include_bytes!("../tests/fixtures/assembly_zip_video.script.bin").to_vec();
        let mut archive =
            include_bytes!("../tests/fixtures/assembly_zip_video.archive.bin").to_vec();
        ...
        assert!(err.contains("ZIP64 or multi-disk"));
    }

    #[test]
    fn rejects_central_entry_disk_start_during_rebase() {
        let mut image = base_image();
        let script = include_bytes!("../tests/fixtures/assembly_zip_video.script.bin").to_vec();
        let mut archive =
            include_bytes!("../tests/fixtures/assembly_zip_video.archive.bin").to_vec();
        ...
        archive[central + 34..central + 36].copy_from_slice(&1u16.to_le_bytes());

        let err = embed_chunks(&mut image, script, archive, base_image_size())
            .expect_err("entry disk start must fail");
        assert!(err.contains("Multi-disk local header reference"));
    }
